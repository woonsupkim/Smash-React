...
export const CHANGELOG = [
  {
    ...
    notes: [
      ...
      'THE NO-CALL FILTER WAS NOT ACTUALLY RUNNING ON THE FORWARD LEDGER. It tested a flag written at lock time, and no ledger row had ever carried one - so every graded row passed a filter that looked correct and did nothing, and 74 coin flips were counted as calls. The rule is now derived from the stated probability wherever it is applied, so moving the threshold moves the whole history with it. The locked-before-play record is 72% of 204 calls, not 68% of 278, and recent form reads 70% over the last 40 rather than 60%.',
      'THE SEASON BENCHMARK WAS PUBLISHED AT 94%. It counted winners across every graded match and divided by the calls only - a numerator from one population over a denominator from another. It is 72%, of 2,381 calls. The engine comparison had a quieter version of the same fault, plotting our model over every call beside the bookmakers over the priced ones under a heading promising "same matches"; scored like for like it is 75% to their 74%.',
      ...
      'AND SOME SMALLER HONESTIES. The daily digest\'s market column said "Bookies\' pick: Swiatek lost" without ever saying whether that was the same player we backed; it now leads with whether they agreed with us. The "on court today" section vanished on days with no locked fixtures, orphaning the section after it, and now names the empty state. During a tournament both the digest and the builder carry the running total from following the recommendation - currently -26.3% across Cincinnati, which is exactly the sort of number this note exists to publish rather than bury.',
    ],
  },
  {
    version: '4.2.4',
