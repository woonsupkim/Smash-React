...
// Monte Carlo simulator uses - the exact number the simulation converges
// to, with zero draw-to-draw noise. ES-module mirror of
// data-pipeline/lib/analyticProb.js (keep the two in sync; engines.test.js
// pins them together with shared fixture values).

// P(server wins a point) given server's [p1..p6] and returner's [.., r3, r4, ..].
export function pointProb(srv, rtn) {
...
export function gameProb(q) {
  const r = 1 - q;
  const deuce = (q * q) / (1 - 2 * q * r);
  return q ** 4 * (1 + 4 * r + 10 * r * r) + 20 * q ** 3 * r ** 3 * deuce;
}

// P(A wins the tiebreak); serve pattern 1-2-2-..., 6-6 pair loop.
export function tiebreakProb(qA, qB, starter) {
...
// P(A wins a set); opening server random, tiebreak at 6-6.
export function setProb(qA, qB) {
...
// P(A wins a set from an arbitrary in-set score. gamesA/gamesB = games
// already won; serverNext = 0 if A serves the next game, 1 if B, null if
// unknown (averages both, mirroring setProb's random opening server).
export function setProbFrom(qA, qB, gamesA = 0, gamesB = 0, serverNext = null) {
  const holdA = gameProb(qA), holdB = gameProb(qB);
  const played = gamesA + gamesB;
  const solve = (srvNext) => {
    // Serve alternates by total games, so the set's opening server is tied
    // to who serves next by parity - and the tiebreak starter needs it.
...
// LIVE match win probability from a mid-match score state - the number the
// broadcast graphic shows. Future sets are priced with the fresh-set
// probability; the current set is priced from its actual games score.
// state: { setsA, setsB, gamesA, gamesB, serverNext (0|1|null) }
export function matchProbLive(probsA, probsB, bestOf = 3, state = {}) {
...
export function matchProb(probsA, probsB, bestOf = 3) {
  const qA = pointProb(probsA, probsB);
  const qB = pointProb(probsB, probsA);
