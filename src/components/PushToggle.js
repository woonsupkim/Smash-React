...
export default function PushToggle() {
  ...
  const enable = async () => {
    ...
    try {
      ...
      const json = s.toJSON();
      // Always anonymous now that there are no accounts: a subscription is a
      // delivery address, nothing more. user_id stays nullable in the schema.
      const { error } = await supabase.from('push_subscriptions').insert({ endpoint: json.endpoint, keys: json.keys, user_id: null });
