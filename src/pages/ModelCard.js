...
const ENGINE_NAMES = { smash: 'Smart Blend', sim: 'Point Sim', elo: 'Form', rank: 'Rankings', upset: 'Hot Streak' };
const GUARD_STATUS = {
  ok: { label: 'On track', cls: 'ok' },
  watch: { label: 'Watch', cls: 'watch' },
  alert: { label: 'Alert', cls: 'alert' },
  insufficient: { label: 'Small sample', cls: 'dim' },
};
...
export default function ModelCard() {
  useDocMeta(
    'The Engine Room: Model Card | Smash',
    'How the Smash prediction engine works: five competing engines, live accuracy, guardrails, and honest limits.'
  );
  const [tr, setTr] = useState(null);
  const [failed, setFailed] = useState(false);
  // Guardrail report - regenerated by every data refresh. Absent until the
  // first refresh after the feature shipped, so the section hides gracefully.
      ...
      {guard?.cells?.length > 0 && (
        <section className="mc-section">
          ...
          <table className="mc-table mc-guard-table">
            ...
            <tbody>
              {guard.cells.map((c) => (
                <tr key={`${c.tour}-${c.surface}`}>
                  <td className="mc-td-label">{c.tour.toUpperCase()} · {c.surface}</td>
                  <td>{ENGINE_NAMES[c.engine] || c.engine}</td>
                  <td>{c.seasonAcc == null ? '–' : `${c.seasonAcc}%`}</td>
                  <td>{c.recentAcc == null ? '–' : `${c.recentAcc}% of last ${c.recentN}`}</td>
                  <td>
                    <span className={`mc-guard-chip ${(GUARD_STATUS[c.status] || GUARD_STATUS.insufficient).cls}`}>
                      {(GUARD_STATUS[c.status] || GUARD_STATUS.insufficient).label}
                    </span>
                  </td>
                </tr>
              ))}
              {guard.forward?.recentN > 0 && (
                <tr>
                  <td className="mc-td-label">Forward test</td>
                  <td>Locked calls</td>
                  <td>–</td>
                  <td>{guard.forward.recentAcc == null ? '–' : `${guard.forward.recentAcc}% of last ${guard.forward.recentN}`}</td>
                  <td>
                    <span className={`mc-guard-chip ${(GUARD_STATUS[guard.forward.status] || GUARD_STATUS.insufficient).cls}`}>
                      {(GUARD_STATUS[guard.forward.status] || GUARD_STATUS.insufficient).label}
                    </span>
                  </td>
                </tr>
              )}
          ...
          {guard.cells.some((c) => c.reasons?.length > 0) && (
            <ul className="mc-list mc-guard-reasons">
              {guard.cells.flatMap((c) =>
                (c.reasons || []).map((r, i) => (
                  <li key={`${c.tour}-${c.surface}-${i}`}>
                    <strong>{c.tour.toUpperCase()} {c.surface}:</strong> {r}
                  </li>
                ))
              )}
            </ul>
          ...
          <p className="mc-fine">
            "Watch" means the recent window runs 8+ points under the season number
            (about one standard deviation over {guard.window} matches); "Alert" means
            a bigger drop, losing to the rankings baseline, or falling below a coin
            flip. Cold streaks happen to honest models - what this board promises is
            that one can never go unnoticed.
          </p>
        </section>
      )}
