...
test('Size Your Risk sits in the Today nav, after the builder', async ({ page }) => {
  await page.goto('/');
  ...
  expect(names).toContain('Size Your Risk');
  const i = names.findIndex((t) => t === 'Size Your Risk');
  expect(names[i - 1]).toBe('The Parlay Builder');
  expect(names[i + 1]).toMatch(/The Draw/);

  await page.locator('.nav-pillar-menu').getByRole('link', { name: 'Size Your Risk' }).click();
